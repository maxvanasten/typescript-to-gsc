...
export const custom_functions = [
            ...
            while_loop(
                ...
                [
                    ...
                    Core.run_function_on_entity(
                        `maps\\mp\\zm_tomb_capture_zones::init_capture_zone()`,
                        `self.recapture_zone`
                    ),
                    Core.wait(0.5),
                ]
    ...
    },
    {
        ...
        lines: [
            Player.set_value(
                "b_challenge_exists",
                'maps\\mp\\zombies\\_zm_challenges::challenge_exists ("zc_zone_captures")'
            ),

            Player.notify("completed_zone_capture"),
            Player.run_function(
                `maps\\mp\\zombies\\_zm_score::player_add_points("bonus_points_powerup", 100)`
            ),
            ...
            if_statement(
                ...
                [
                    Player.run_function(
                        `maps\\mp\\zombies\\_zm_challenges::increment_stat("zc_zone_captures")`
                    ),
                ]
            ),

            Player.run_function(
                `maps\\mp\\zombies\\_zm_stats::increment_client_stat("tomb_generator_captured", 0)`
            ),
            Player.run_function(
                `maps\\mp\\zombies\\_zm_stats::increment_player_stat("tomb_generator_captured")`
            ),
    ...
    },
    {
        ...
        lines: [
            Player.set_value(
                "random_perk",
                Core.random_from_array("self.perk_list")
            ),
...
export const init_functions = [
    ...
    Core.replace_function_with_custom(
        "maps\\mp\\zm_tomb_capture_zones::get_generator_capture_start_cost",
        "get_generator_capture_start_cost"
    ),
    Core.replace_function_with_custom(
        "maps\\mp\\zm_tomb_capture_zones::reward_players_in_capture_zone",
        "reward_players_in_capture_zone"
    ),
    Core.replace_function_with_custom(
        "maps\\mp\\zm_tomb_capture_zones::get_progress_rate",
        "get_progress_rate"
    ),
